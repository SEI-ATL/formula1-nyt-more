...
class FormulaOne {
  ...
  /**
   * 
   * @param {{player: String, team: String, country: String, points: Number}} Driver
   */
  constructor({ player, team, country, points }) {
    this.player = player
    this.team = team
  ...
  /**
   * 
   * @param {(drivers|{})} arrayOfDrivers - An array of driver objects or an individual driver object
   */
  static createDrivers = (arrayOfDrivers) => []
    .concat(arrayOfDrivers)
    .map(driver => new FormulaOne(driver))
}

const arrayOfDrivers = FormulaOne.createDrivers(drivers[0])
