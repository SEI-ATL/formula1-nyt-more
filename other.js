...
class newDriver {
    constructor(player, rank, team, number, points, country) {
        this.player = player;
        this.rank = rank;
        this.team = team
        this.number = number;
...
const me = new newDriver('corey wilson', 25, 'Alfa Romeo', 0, 'United States')
const notMe = new newDriver('cleo robinson', 26, 'Ferrari', 0, 'Jamaica')
drivers2.push(me)
drivers2.push(notMe)

console.log(drivers2)

// Driver(Object.values(currentDriver)[0]

let newPromise = new Promise(function(resolve, reject) {
    let x = 26
    if (x <= 24) {
        console.log("We do not have a player that rank")
        return reject()
    } else {
        return resolve()
    }
